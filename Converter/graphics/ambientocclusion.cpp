...
PixelInputType main(VertexInputType input)
{
	...
	return output;
		...
		void WriteBuffer(ID3D11DeviceContext3* context, ID3D11Buffer* buffer, void* data, unsigned dataSize)
		{
		...
		{
			WriteBuffer(context, m_normalDepthMapVSBuffer.Get(), data, m_normalDepthMapVSBufferSize);
		}
		void AmbientOcclusion::WriteScreenQuadVSBuffer(ID3D11DeviceContext3* context, void* data)
		{
			WriteBuffer(context, m_screenQuadVSBuffer.Get(), data, m_screenQuadVSBufferSize);
		}
		void AmbientOcclusion::WriteOcclusionMapPSBuffer(ID3D11DeviceContext3* context)
		{
			WriteBuffer(context, m_occlusionMapPSBuffer.Get(), &m_occlusionMapPSBufferData, m_occlusionMapPSBufferSize);
		}
		void AmbientOcclusion::WriteBlurPSBuffer(ID3D11DeviceContext3* context, void* data)
		{
			WriteBuffer(context, m_blurPSBuffer.Get(), data, m_blurPSBufferSize);
		}
		...
		void AmbientOcclusion::BlurOcclusionMap(Graphics& graphics)
		{
			...
			m_screenQuad->RenderGroup(graphics, 0, 6);
		...
		void AmbientOcclusion::SetAsRenderTarget(Graphics& graphics, Camera& camera)
		{
			...
			mth::float4x4 worldMatrix = mth::float4x4::Identity();
			mth::float4x4 matrixBuffer[] =
			{
				worldMatrix,
				camera.ViewMatrix() * worldMatrix,
				camera.ProjectionMatrix()
			};
			WriteNormalDepthMapVSBuffer(context, matrixBuffer);
		}
